        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <!-- Form Update Profile -->
                    <form action="{{ route($user->prefix . 'profile-handle') }}" method="POST" enctype="multipart/form-data">
                        @csrf
    ...
    if (input.files && input.files[0]) {
        ...
        if (!allowedTypes.includes(file.type)) {
            Swal.fire({
                ...
                text: 'File harus berupa gambar (JPEG, JPG, PNG, atau GIF)',
...
// Tambahan: Show loading state saat upload
document.querySelector('form').addEventListener('submit', function(e) {
    const submitBtn = this.querySelector('button[type="submit"]');
    
    // Validation untuk alamat KTP jika user mengisi tab alamat
    const alamatKtpTextarea = this.querySelector('textarea[name="alamat_ktp[alamat_lengkap]"]');
    const alamatTab = document.querySelector('a[href="#alamat"]');
    
    // Jika ada input di form alamat KTP yang diisi, maka alamat lengkap wajib diisi
    const alamatKtpInputs = this.querySelectorAll('input[name^="alamat_ktp"], textarea[name^="alamat_ktp"]');
    let hasAlamatKtpData = false;
    
    alamatKtpInputs.forEach(input => {
        if (input.value.trim() !== '' && input.name !== 'alamat_ktp[id]' && input.name !== 'alamat_ktp[tipe]') {
            hasAlamatKtpData = true;
        }
    });
    
    if (hasAlamatKtpData && alamatKtpTextarea.value.trim() === '') {
        e.preventDefault();
        Swal.fire({
            icon: 'warning',
            title: 'Data Tidak Lengkap',
            text: 'Alamat lengkap KTP wajib diisi jika Anda mengisi data alamat KTP lainnya.',
            confirmButtonColor: '#667eea'
        }).then(() => {
            // Pindah ke tab alamat dan focus ke textarea
            alamatTab.click();
            setTimeout(() => {
                alamatKtpTextarea.focus();
                alamatKtpTextarea.scrollIntoView({ behavior: 'smooth', block: 'center' });
            }, 300);
        });
        return;
    }
...
async function ajaxDelete(url) {
  ...
  if (!res.ok) {
    ...
    const msg = js.message || 'Gagal menghapus data';
    throw new Error(msg);
  }
  return true;
}
...
function removePendidikan(index) {
  Swal.fire({
    ...
    const url = idVal
      ? `{{ route($user->prefix."profile.delete-pendidikan", ['id' => '__ID__']) }}`.replace('__ID__', idVal)
      : null;
...
function removeKeluarga(index) {
  Swal.fire({
    ...
    const url = idVal
      ? `{{ route($user->prefix."profile.delete-keluarga", ['id' => '__ID__']) }}`.replace('__ID__', idVal)
      : null;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Reset preview photo if input is empty
    const photoInput = document.querySelector('input[name="photo"]');
